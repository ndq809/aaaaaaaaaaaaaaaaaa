...
var append = function append(ts, str) {
  ...
  if (endTerm!=undefined && endTerm.whitespace.after) {
...
var value_step = function value_step(ts) {
  ...
  ts.match('#Cardinal #Plural').lastTerm().tag('Unit', 'cardinal-plural'); //5th book
...
var corrections = function corrections(ts) {
  ...
  if (ts.has('#Determiner')) {
    ...
    ts.match('(some [#Verb] #Plural').tag('Noun', 'correction-determiner6'); //the orange.
