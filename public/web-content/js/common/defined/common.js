...
function initEvent(){
    ...
    if($('.right-tab .tab-content').length!=0){
            $('.right-tab .tab-content').height( $('.col-md-pull-4').height()-$('.select-group').height()-22);
    }
...
function setFooter(){
    var temp=$('.header-content').height()+$('.middle-content').height()+$('.bottom-content').height();
    if(temp<$('body').height()){
...
function setNextItem(item_of_table='tr') {
    ...
    $(".right-tab .tab-content").animate({ 
      scrollTop: nextItem.height()*i
    }, 200);
    return nextItem.attr("id");
}

function setPreviousItem(item_of_table="tr") {
    listItem = $(selectedTab + " table tbody "+item_of_table);
    currentSelectItem = $(selectedTab + " table .activeItem");
    itemId = currentSelectItem.attr("id");
    var nextItem;
...
function setCollapseIcon(){
    $(".collapse-icon").append('<i class="glyphicon glyphicon-menu-down" style="float: right;margin-right:2px;"></i');
}