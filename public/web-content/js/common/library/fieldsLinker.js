...
var drawColumnsContentA = function(){
	...
	$(factory).find(".FL-main .FL-left li").off("mousedown").on("mousedown",function(e){
		...
		if($(factory).find(".FL-main .FL-right li.activeTag").length!=0 && $(factory).find(".FL-main .FL-left li.activeTag").length==0){
			move.offsetB = $(factory).find('.FL-main .FL-right li.activeTag').data("offset");
			move.nameB = $(factory).find('.FL-main .FL-right li.activeTag').data("name");
			...
			move.offsetB = -1;
			move.nameB = -1;
			$(factory).find(".FL-main .FL-left li").removeClass('activeTag');
			$(factory).find(".FL-main .FL-right li").removeClass('activeTag');
			$(this).addClass('activeTag');
		}
	});
	...
	$(factory).find(".FL-main .FL-right li").off("mouseup").on("mouseup", function (e) {
		...
		if(move != null){ // no drag
			...
			if($(factory).find(".FL-main .FL-right li.activeTag").length==0 && $(factory).find(".FL-main .FL-left li.activeTag").length!=0){
				move = {};
				move.offsetA = $(factory).find('.activeTag').data("offset");
				move.nameA = $(factory).find('.activeTag').data("name");
				move.offsetB = $(this).data("offset");
				move.nameB = $(this).data("name");
				var infos =  JSON.parse(JSON.stringify(move));
				move = null;
	...
	$.fn.fieldsLinker = function(action,input) {
	    ...
	    if (action == "init") {
            ...
            if (!onError) {
                ...
                links = JSON.parse(JSON.stringify(linksByName));
