...
(function ($) {
    $.fn.Exponent = function (params) {
        ...
        function calculate() {
            ...
            if (options.data.length > 0) {
                for (i = 0; i < options.data.length; i++) {
                    lineData[i] = [];
                    for (j = 0; j < options.target; j += 0.1) {
                        lineData[i].push(Math.pow(options.data[i], j));
                    }
                }
            }
            ...
            //计算最大值与最小值
            var dataArr = lineData.join(',').split(',');
            ...
            if (options.axis.manualY) {
                ...
                for (i = 0; i < options.axis.y + 1; i++) {
                    ...
                    value = options.isPercent ? parseFloat((value * 100).toFixed(3)) + '%' : value;
                    valueY.push(value);
        ...
        function drawBaseArea() {
            ...
            //绘制与X轴平行的线与文字
            for (i = (options.axis.isShowMinY ? 0 : 1); i < options.axis.y + 1; i++) {
                var points = [{
                    x: top_left.x,
