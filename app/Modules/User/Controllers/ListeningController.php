...
class ListeningController extends ControllerUser
{
	/**
    ...
    {
        if (\Session::get('mission') != null && \Session::get('mission')['link'] == '/listening') {
            return view('User::listening.index');
        }
        $param                 = $request->all();
    ...
    {    $param = [];
        if (\Session::get('mission') != null && \Session::get('mission')['link'] == '/listening') {
            ...
            $param['user_id']    = isset(Auth::user()->account_id) ? Auth::user()->account_id : '';
            $data                = Dao::call_stored_procedure('SPC_LISTENING_LST3', $param);
        } else {
            $param            = $request->all();
            ...
            $data   = Dao::call_stored_procedure('SPC_LISTENING_LST2', $param);
        }
        $data   = CommonUser::encodeID($data);
        $view1  = view('User::listening.right_tab')->with('data', $data[2])->render();
        $view2  = view('User::listening.main_content')->with('data', $data)->render();
        $result = array(
            'status'     => 200,
            'voca_array' => $data[2],
            'listen_array' => $data[5],
