...
class RegisterController extends Controller
{
     ...
     public function postUpload()
    {
        ...
        $image = $manager->make( $photo )->encode('jpg')->save(storage_path(env('UPLOAD_PATH'))  . $filename_ext );
        ...
        return Response::json([
            'status'    => 'success',
            'url'       => 'storage/uploads/' . $filename_ext,
            'width'     => $image->width(),
            'height'    => $image->height()
        ], 200);
    }
    ...
    public function postCrop()
    {
        ...
        $image->resize($imgW, $imgH)
            ...
            ->save(storage_path(env('UPLOAD_PATH')) . 'cropped-' . $filename);
        ...
        return Response::json([
            'status' => 'success',
            'url' => env('URL') . 'storage/uploads/cropped-' . $filename
        ], 200);
