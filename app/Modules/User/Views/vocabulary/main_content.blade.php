...
@if(isset($data)&&$data[2][0]['vocabulary_id'] != '')
    @foreach($data[2] as $index => $row)
        <div class="col-xs-12 no-padding vocabulary-box hidden" word-id="{{$row['row_id']}}">
            ...
            <input type="text" name="" class="form-control input-sm vocal-spell" value="/{{$row['spelling']}}/" readonly="readonly">
            <input type="text" name="" class="form-control input-sm vocal-div" value="{{$row['vocabulary_div']}}" readonly="readonly">
            <textarea class="form-control input-sm vocal-mean" readonly="readonly" rows="2">{{$row['mean']}}</textarea>
            <textarea class="form-control input-sm vocal-explain" readonly="readonly" rows="2">{{$row['explain']}}</textarea>
