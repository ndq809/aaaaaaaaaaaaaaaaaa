                    ...
                    @foreach($data as $index => $row)
                        @if($row['remembered']==0)
                            <tr id="{{$row['row_id']}}">
                                ...
                                <td >
                                    <button class="btn btn-sm btn-default {{$raw_data[0][0]['btn-remember']==1?'btn-remember':'btn-disabled'}}" {{$raw_data[0][0]['btn-remember']==1?'':'rank='.$raw_data[0][0]['btn-remember']}}>Theo dõi</button>
                                </td>
                            </tr>
                            @php($count = $count + 1)
                        @endif
                    ...
                    @foreach($data as $index => $row)
                        @if($row['remembered']==1)
                            <tr id="{{$row['row_id']}}">
                                ...
                                <td >
                                    <button class="btn btn-sm btn-default {{$raw_data[0][0]['btn-forget']==1?'btn-forget':'btn-disabled'}}" {{$raw_data[0][0]['btn-forget']==1?'':'rank='.$raw_data[0][0]['btn-forget']}}>{{$row['del_flg']==0?'Bỏ theo dõi':'Xóa'}}</button>
                                </td>
                            </tr>
                            @php($count = $count + 1)
                        @endif
