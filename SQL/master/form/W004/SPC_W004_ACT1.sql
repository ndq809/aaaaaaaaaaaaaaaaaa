IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPC_W004_ACT1]') AND type IN (N'P', N'PC'))
DROP PROCEDURE [dbo].[SPC_W004_ACT1]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SPC_W004_ACT1]

     @P_en_title			NVARCHAR(MAX)		= ''
,    @P_vi_title  			NVARCHAR(MAX)		= ''
,    @P_en_content  		NTEXT				= ''
,    @P_vi_content  		NTEXT				= ''
...
BEGIN
	...
	,	@w_program_id			NVARCHAR(50)		= 'W004'
	,	@w_prs_prg_nm			NVARCHAR(50)		= N'Import bài viết đọc hiểu'
	...
	IF NOT EXISTS(SELECT 1
			  ...
			  AND LOWER(M002.catalogue_div)	=	5
			  AND M002.del_flg			=	0
	)
	BEGIN
		--
