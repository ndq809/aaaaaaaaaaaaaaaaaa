...
BEGIN
	...
	,   @w_prs_prg				VARCHAR(10)		= 'S002'
	,   @w_prs_prg_nm			VARCHAR(50)		= 'Danh sách tài khoản'
	...
	BEGIN TRY
		SET @w_prs_result = 'OK'
		--
		UPDATE S001 SET S001.del_flg = 1
		WHERE S001.account_id IN (
		SELECT
			role_id		=	T.C.value('@id', 'nvarchar(15)')
		FROM @P_account_id_xml.nodes('row') T(C))
