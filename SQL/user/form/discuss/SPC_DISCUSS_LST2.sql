IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPC_DISCUSS_LST2]') AND type IN (N'P', N'PC'))
/****** Object:  StoredProcedure [dbo].[SPC_M001L_FND1]    Script Date: 2017/11/23 16:46:46 ******/
DROP PROCEDURE [dbo].[SPC_DISCUSS_LST2]
GO
...
CREATE PROCEDURE [dbo].[SPC_DISCUSS_LST2]
...
BEGIN
	...
	BEGIN
		...
		FROM (
		  SELECT #DISCUSS.row_id, ROW_NUMBER() OVER(ORDER BY #DISCUSS.row_id ASC) AS new_row_id
		  FROM #DISCUSS
		  ) temp
	END
	ELSE
	...
	FROM F004
	INNER JOIN #DISCUSS
	ON F004.target_id				= #DISCUSS.post_id
	AND F004.reply_id IS NULL
	AND F004.screen_div = 8
	)TEMP
	WHERE
	TEMP.count_row_id < 6
	...
	INNER JOIN #DISCUSS
	ON M004.post_id = #DISCUSS.post_id
	WHERE 
		M004.question_id IN (SELECT TOP 5 M004.question_id FROM M004 WHERE M004.post_id IN(SELECT #DISCUSS.post_id FROM #DISCUSS) ORDER BY NEWID())
	...
	AND F009.briged_div = 2
	INNER JOIN #DISCUSS 
	ON F009.briged_id = #DISCUSS.briged_id
